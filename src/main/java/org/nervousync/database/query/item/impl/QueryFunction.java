...
public final class QueryFunction extends QueryItem {
	...
	@XmlElementRefs({
			@XmlElementRef(name = "column_param", type = QueryColumn.class),
			@XmlElementRef(name = "constant_param", type = QueryConstant.class),
			@XmlElementRef(name = "function_param", type = QueryFunction.class)
	})
     ...
     *                       <span class="zh-CN">函数参数数组</span>
     */
    public void setFunctionParams(List<QueryItem> functionParams) {
    ...
    public boolean match(final QueryItem queryItem) {
        ...
        if (ItemType.FUNCTION.equals(queryItem.getItemType())
                && this.sqlFunction.equalsIgnoreCase(((QueryFunction) queryItem).getSqlFunction())) {
